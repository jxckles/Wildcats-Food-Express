...
const MainAdminInterface = () => {
  ...
  useEffect(() => {
    axios
      .get("https://wildcats-food-express.onrender.com/api/admin")
      .then((res) => {
        if (res.data.valid) {
  ...
  useEffect(() => {
    const newSocket = io("https://wildcats-food-express.onrender.com/api");
    setSocket(newSocket);
  ...
  const fetchMenuItems = async () => {
    try {
      const response = await axios.get("https://wildcats-food-express.onrender.com/api/menu");
      const menuData = response.data.map((item) => ({
        ...item,
        image: item.image
          ? `https://wildcats-food-express.onrender.com/api/Images/${item.image.split("\\").pop()}`
          : null,
      }));
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await axios.get(
        `https://wildcats-food-express.onrender.com/api/orders?userId=${userId}`
      );
  ...
  const fetchClientOrders = async () => {
    try {
      const response = await axios.get("https://wildcats-food-express.onrender.com/api/clientorders");
  ...
  const handleStatusChangeClient = async (orderId, newStatus) => {
    try {
      if (newStatus === "Cancelled") {
        await axios.delete(`https://wildcats-food-express.onrender.com/api/clientorders/${orderId}`);
        setCustomerOrders((prevOrders) =>
          prevOrders.filter((order) => order._id !== orderId)
        );
      } else {
        await axios.put(
          `https://wildcats-food-express.onrender.com/api/clientorders/${orderId}/status`,
          { status: newStatus }
        );
        setCustomerOrders((prevOrders) =>
          prevOrders.map((order) =>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentItem._id) {
        await axios.put(
          `https://wildcats-food-express.onrender.com/api/menu/${currentItem._id}`,
          formData,
          {
            headers: {
        ...
        await axios.post("https://wildcats-food-express.onrender.com/api/menu", formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
  ...
  const handleDelete = async (_id) => {
    try {
      const response = await axios.delete(`https://wildcats-food-express.onrender.com/api/menu/${_id}`);
  ...
  const handleStatusChange = async (orderId, newStatus, isOnlineOrder) => {
    try {
      ...
      const response = await axios.put(
        `https://wildcats-food-express.onrender.com/api/orders/${orderId}/status`,
        {
          status: newStatus,
        }
      );
  ...
  const handleGcashNumberUpdate = async () => {
    try {
      const response = await axios.post("https://wildcats-food-express.onrender.com/api/update-gcash-number", { gcashNumber });
  ...
  const handleLogout = async () => {
    try {
      await axios.post(
        "https://wildcats-food-express.onrender.com/api/logout",
        {},
    ...
    if (file) {
      ...
      try {
        const response = await axios.post(
          "https://wildcats-food-express.onrender.com/api/upload-qr-code",
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
        ...
        if (response.data.qrCodeUrl) {
          setQrCodeImage(`https://wildcats-food-express.onrender.com/api${response.data.qrCodeUrl}`);
        }
  ...
  const handleRemoveQRCode = async () => {
    try {
      await axios.delete("https://wildcats-food-express.onrender.com/api/remove-qr-code");
      setQrCodeImage(null);
  ...
  const fetchQRCode = async () => {
    try {
      const response = await axios.get("https://wildcats-food-express.onrender.com/api/get-qr-code");
      if (response.data.qrCodeUrl) {
        setQrCodeImage(`https://wildcats-food-express.onrender.com/api${response.data.qrCodeUrl}`);
      } else {
        setQrCodeImage(null);
  ...
  const fetchHistoryOrders = async () => {
    try {
      ...
      const response = await axios.get(
        `https://wildcats-food-express.onrender.com/api/history-orders?userId=${userId}`
      );
                  ...
                  <tr key={order._id}>
                    ...
                    <td>
                      <td style={{ border: "none" }}>
                        {order.receiptPath ? (
                          <a
                            href={`https://wildcats-food-express.onrender.com/api${order.receiptPath}`} // Updated to use the correct port
                            target="_blank"
