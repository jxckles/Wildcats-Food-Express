...
const MainAdminInterface = () => {
  ...
  useEffect(() => {
    axios
      .get("https://wildcats-food-express-deploy2.vercel.app/admin")
      .then((res) => {
        if (res.data.valid) {
  ...
  const fetchClientOrders = async () => {
    try {
      const response = await axios.get("https://wildcats-food-express-deploy2.vercel.app/clientorders");
  ...
  const handleStatusChangeClient = async (orderId, newStatus) => {
    try {
      if (newStatus === "Cancelled") {
        await axios.delete(`https://wildcats-food-express-deploy2.vercel.app/clientorders/${orderId}`);
        setCustomerOrders((prevOrders) =>
          prevOrders.filter((order) => order._id !== orderId)
        );
      } else {
        await axios.put(
          `https://wildcats-food-express-deploy2.vercel.app/clientorders/${orderId}/status`,
          { status: newStatus }
        );
        setCustomerOrders((prevOrders) =>
          prevOrders.map((order) =>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentItem._id) {
        await axios.put(
          `https://wildcats-food-express-deploy2.vercel.app/menu/${currentItem._id}`,
          formData,
          {
            headers: {
        ...
        await axios.post("https://wildcats-food-express-deploy2.vercel.app/menu", formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
  ...
  const handleDelete = async (_id) => {
    try {
      const response = await axios.delete(`https://wildcats-food-express-deploy2.vercel.app/menu/${_id}`);
  ...
  const handleStatusChange = async (orderId, newStatus, isOnlineOrder) => {
    try {
      ...
      const response = await axios.put(
        `https://wildcats-food-express-deploy2.vercel.app/orders/${orderId}/status`,
        {
          status: newStatus,
        }
      );
  ...
  const handleGcashNumberUpdate = async () => {
    try {
      const response = await axios.post("https://wildcats-food-express-deploy2.vercel.app/update-gcash-number", { gcashNumber });
    ...
    if (file) {
      ...
      try {
        const response = await axios.post(
          "https://wildcats-food-express-deploy2.vercel.app/upload-qr-code",
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
  ...
  const handleRemoveQRCode = async () => {
    try {
      await axios.delete("https://wildcats-food-express-deploy2.vercel.app/remove-qr-code");
      setQrCodeImage(null);
                  ...
                  <tr key={order._id}>
                    ...
                    <td>
                      <td style={{ border: "none" }}>
                        {order.receiptPath ? (
                          <a
                            href={`https://wildcats-food-express-deploy2.vercel.app${order.receiptPath}`} // Updated to use the correct port
                            target="_blank"
